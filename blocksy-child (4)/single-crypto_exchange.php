<?php
/**
 * Шаблон одиночной карточки криптобиржи — uztrading.net
 * Файл: wp-content/themes/blocksy-child/single-crypto_exchange.php
 */
...
$uzt_links = array(
	'methodology' => home_url( '/metodologiya/' ),
	'exchanges'   => home_url( '/crypto-exchanges/' ),
	'faq'         => home_url( '/faq/' ),
	'risk'        => home_url( '/legal/risk-disclaimer/' ),
	'author'      => home_url( '/author/konstantin-ivanov/' ),
);

while ( have_posts() ) : the_post();
	$name        = get_the_title();
	$rating      = (float) get_field( 'cex_rating' );
	$logo_wide_raw    = get_field( 'cex_logo_wide' );
	$logo_compact_raw = get_field( 'cex_logo_compact' );
	...
	$affiliate   = get_field( 'cex_affiliate_url' );
	$official    = get_field( 'cex_official_url' );
	$legal       = get_field( 'cex_legal_entity' );
	$founded     = get_field( 'cex_founded' );
	$countries   = get_field( 'cex_countries' );
	$licenses    = get_field( 'cex_licenses' );
	$spot_fee    = get_field( 'cex_spot_fee' );
	$futures_fee = get_field( 'cex_futures_fee' );
	$p2p_fee     = get_field( 'cex_p2p_fee' );
	$leverage    = get_field( 'cex_leverage' );
	$staking     = get_field( 'cex_staking' );
	$copytrading = get_field( 'cex_copytrading' );
	$p2p         = get_field( 'cex_p2p' );
	$coins       = get_field( 'cex_coins_count' );
	$fiat        = get_field( 'cex_fiat' );
	$uzs         = get_field( 'cex_uzs_support' );
	$por         = get_field( 'cex_por' );
	$hacks       = get_field( 'cex_hacks' );
	$reg_status  = get_field( 'cex_reg_status' );
	$uz_access   = get_field( 'cex_uz_access' );
	$trust       = get_field( 'cex_trust_score' );
	$volume      = get_field( 'cex_daily_volume' );
	$support     = get_field( 'cex_support_lang' );
	$min_dep     = get_field( 'cex_min_deposit' );

	$pros = uzt_list_items( 'cex_pros' );
	$cons = uzt_list_items( 'cex_cons' );
	$rating_pct = $rating > 0 ? min( 100, ( $rating / 5 ) * 100 ) : 0;
	$go_url = $affiliate ? $affiliate : ( $official ? $official : '#' );
?>
<main class="uzt-broker uzt-cex" id="cex-top">
  ...
  <section class="uzt-hero">
    ...
    <?php endif; ?>
    <h1 class="uzt-hero__title"><?php echo esc_html( $name ); ?>: обзор криптобиржи <?php echo esc_html( date_i18n( 'Y' ) ); ?></h1>
    ...
    <div class="uzt-badges">
      <?php if ( $uzs === 'Да' ) : ?><span class="uzt-badge uzt-badge--ok">✓ Поддержка UZS</span><?php elseif ( $uzs === 'частично' ) : ?><span class="uzt-badge">UZS частично</span><?php endif; ?>
      <?php if ( $trust ) : ?><span class="uzt-badge">Trust Score <?php echo esc_html( $trust ); ?></span><?php endif; ?>
    </div>

    <div class="uzt-kpi">
      <div class="uzt-kpi__item"><span>Спот Maker/Taker</span><strong><?php echo esc_html( $spot_fee ?: '—' ); ?></strong></div>
      <div class="uzt-kpi__item"><span>Монет</span><strong><?php echo esc_html( $coins ?: '—' ); ?></strong></div>
      <div class="uzt-kpi__item"><span>Мин. депозит</span><strong><?php echo esc_html( $min_dep ?: '—' ); ?></strong></div>
    </div>

    <a class="uzt-cta" href="<?php echo esc_url( $go_url ); ?>" target="_blank" rel="sponsored nofollow noopener">Перейти на биржу →</a>
    <p class="uzt-cta__note">Партнёрская ссылка</p>

    <p class="uzt-risk">⚠ Торговля криптовалютами связана с высоким риском полной потери средств. Криптоактивы крайне волатильны и в Узбекистане регулируются НАПП — проверяйте доступность сервиса.</p>
  </section>

  <section class="uzt-verdict">
  <?php the_content(); ?>

  <!-- Day 3.5.CE: Auto-summary отзывов (только если count > 0) -->
  <?php
  $uzt_reviews_count = 0;
  ...
  <section class="uzt-conditions">
    <h2>Комиссии и условия</h2>
    <table>
      <?php
      $cond = array(
        'Спот (Maker/Taker)'     => $spot_fee,
        'Фьючерсы (Maker/Taker)' => $futures_fee,
        'Комиссия P2P'           => $p2p_fee,
        'Плечо'                  => $leverage,
        'Стейкинг / Earn'        => $staking,
        'Копитрейдинг / боты'    => $copytrading,
        'P2P-площадка'           => $p2p,
        'Мин. депозит'           => $min_dep,
      );
  ...
  <section class="uzt-reg">
    <h2>Регуляция и безопасность</h2>
    <table>
      <?php
      $reg = array(
        'Лицензии (по странам)'   => $licenses,
        'Регуляторный статус'     => $reg_status,
        'Страховой фонд / PoR'    => $por,
        'История взломов'         => $hacks,
      );
  ...
  <section class="uzt-pay">
    <h2>Доступность в Узбекистане</h2>
    <table>
      <?php
      $uzb = array(
        'Доступность в Узбекистане' => $uz_access,
        'Поддержка UZS'             => $uzs,
        'Поддержка RU/UZ'           => $support,
        'Фиатные валюты'            => $fiat,
      );
      foreach ( $uzb as $k => $v ) { if ( $v !== '' && $v !== null ) echo '<tr><th>' . esc_html( $k ) . '</th><td>' . esc_html( $v ) . '</td></tr>'; }
      ?>
    </table>
  </section>

  <?php $faqs = uzt_cex_faq_items( 8 ); if ( $faqs ) : ?>
...
<div class="uzt-reviews-context" role="note" aria-label="Как формируется этот блок отзывов">
	...
	<div class="uzt-reviews-context__body">
		<h4 class="uzt-reviews-context__title">Как формируется этот блок отзывов</h4>
...
<!-- НАЧАЛО: Отзывы пользователей (Site Reviews bridge v1.3.0, Day 3.5.CE) -->
<?php
$uzt_reviews_title = 'Отзывы пользователей о ' . $name;
...
?>
<!-- КОНЕЦ: Отзывы пользователей -->

<!-- Блок "Похожие криптобиржи" -->
  <section class="uzt-similar" style="margin: 40px 0;">
    <h2>Похожие криптобиржи</h2>
    <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 20px;">
      <?php
      $similar_query = new WP_Query( array(
        'post_type'           => 'crypto_exchange',
        'posts_per_page'      => 4,
      ...
      if ( $similar_query->have_posts() ) :
        while ( $similar_query->have_posts() ) : $similar_query->the_post();
          $sim_rating = (float) get_field( 'cex_rating' );
          $sim_logo   = uzt_img_url( get_field( 'cex_logo_compact' ) ?: get_field( 'cex_logo_wide' ) );
      ?>
  ...
  <section class="uzt-disclaimer">
    <p>Операции с криптовалютами связаны с высоким риском и могут привести к потере всего капитала. Криптоактивы не являются законным платёжным средством и слабо регулируются.</p>
    <p>uztrading.net получает партнёрское вознаграждение от площадок при переходе по ссылкам на этой странице. Это не влияет на нашу оценку — см. <a href="<?php echo esc_url( $uzt_links['methodology'] ); ?>">методологию</a>.</p>
  ...
  <nav class="uzt-footlinks" aria-label="Полезные ссылки" style="margin-top: 20px; padding-top: 20px; border-top: 1px solid #e0e0e0; display: flex; gap: 15px; flex-wrap: wrap; font-size: 14px; justify-content: center;">
    ...
    <a href="<?php echo esc_url( $uzt_links['exchanges'] ); ?>" style="color: #0056b3; text-decoration: none;">Все криптобиржи</a> |
